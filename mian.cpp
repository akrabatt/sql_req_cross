...
int main() {
    ...
    if (exitCode) { // Проверяем успешность открытия базы данных
    ...
    if (exitCode != SQLITE_OK) { // Проверяем успешность создания таблицы
    ...
    if (exitCode != SQLITE_OK) { // Проверяем успешность вставки данных
        std::cerr << "SQL error: " << errorMessage; // Выводим сообщение об ошибке
        sqlite3_free(errorMessage); // Освобождаем память, занятую errorMessage
    } else {
